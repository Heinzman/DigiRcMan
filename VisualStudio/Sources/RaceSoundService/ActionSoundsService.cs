...
namespace Elreg.RaceSoundService
{
    public class ActionSoundsService
    {
        ...
        public ActionSoundsService(SoundOptionsService soundOptionsService, DriversService driversService,SoundMixer soundMixer, IRaceModel raceModel)
        {
        ...
        public Queue<ActionSound> GetSoundOptionBufferQueue(ActionSoundType type, Lane lane)
        {
            Queue<ActionSound> soundOptionBufferQueue = new Queue<ActionSound>();
            lock (Locker)
            {
        ...
        public WaveOutEvent GetWaveOutEventOf(string fileName, bool varyFrequency)
        {
        ...
        private void AddSoundToQueue(Lane lane, Queue<ActionSound> soundOptionBufferQueue, ActionSound actionSound)
        {
            try
            {
                _specialSoundHandler.AddSpecialSounds(soundOptionBufferQueue, actionSound, lane);
                if (actionSound.WaveOutEvent != null)
                    soundOptionBufferQueue.Enqueue(actionSound);
            }
            catch (Exception ex)
            {
                ErrorLog.LogError(false, ex);
                    ...
                    ActionSound actionSound = new ActionSound
                                                  {
                                                      ...
                                                      WaveOutEvent = GetWaveOutEvent(soundOption, soundOptionList.VaryFrequency),
        ...
        private WaveOutEvent GetWaveOutEvent(SoundOption soundOption, bool varyFrequency)
        {
            string soundFilename = SystemHelper.GetAbsolutePath(soundOption.SoundPath);
            return GetWaveOutEventOf(soundFilename, varyFrequency);
        }
